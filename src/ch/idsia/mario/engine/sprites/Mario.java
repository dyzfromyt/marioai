...
//        ++numberOfAttempts;
    ...
    private Mario(LevelScene world, int dummy)
    {
        super(world);
    }
    ...
    public void move()
    {
        ...
        // Cheats:
        if (GlobalOptions.PowerRestoration && keys[KEY_SPEED] && (!large || !fire))
            setLarge(true, true);
...
//            }
        ...
        if (sliding)
        {
            for (int i = 0; i < 1; i++)
            {
                world.addSprite(new Sparkle(world, (int) (x + Math.random() * 4 - 2) + facing * 8, (int) (y + Math.random() * 4) - 24, (float) (Math.random() * 2 - 1), (float) Math.random() * 1, 0, 1, 5));
            }
            ya *= 0.5f;
        }
    ...
    private void calcPic()
    {
        ...
        if (onGround && ((facing == -1 && xa > 0) || (facing == 1 && xa < 0)))
        {
            ...
            if (xa > 3 || xa < -3)
            {
                for (int i = 0; i < 3; i++)
                {
                    world.addSprite(new Sparkle(world, (int) (x + Math.random() * 8 - 4), (int) (y + Math.random() * 4), (float) (Math.random() * 2 - 1), (float) Math.random() * -1, 0, 1, 5));
                }
            }
        }
        ...
        if (((Level.TILE_BEHAVIORS[block & 0xff]) & Level.BIT_PICKUPABLE) > 0)
        {
            ...
            for (int xx = 0; xx < 2; xx++)
                for (int yy = 0; yy < 2; yy++)
                    world.addSprite(new Sparkle(world, x * 16 + xx * 8 + (int) (Math.random() * 8), y * 16 + yy * 8 + (int) (Math.random() * 8), 0, 0, 0, 2, 5));
        }
...
//        if (deathTime > 0 || world.paused) return;
    ...
    {
        Mario result = new Mario((LevelScene) parent, 1);
        Sprite.makeTrainingCopy(parent, result, this);


        result.keys = keys;
        result.cheatKeys = cheatKeys;
        result.runTime = runTime;
        result.wasOnGround = wasOnGround;
        result.onGround = onGround;
        result.mayJump = mayJump;

        result.ducking = ducking;
        result.sliding = sliding;
        result.jumpTime = jumpTime;
        result.xJumpSpeed = xJumpSpeed;
        result.yJumpSpeed = yJumpSpeed;

        result.canShoot = canShoot;
        result.width  = width;
        result.height  = height;
        result.facing  = facing;

        result.powerUpTime = powerUpTime; // exclude pause for rendering changes
        result.xDeathPos= xDeathPos;
        result.yDeathPos= yDeathPos;
        result.deathTime = deathTime;
        result.winTime = winTime;

        result.invulnerableTime = invulnerableTime;
        result.lastLarge = lastLarge;
        result.lastFire= lastFire;
        result.newLarge= newLarge;
        result.newFire = newFire;
        result.carried = null;
        ...
        result.gainedMushrooms  = gainedMushrooms;
        result.gainedFlowers  = gainedFlowers;
        result.isMarioInvulnerable  = isMarioInvulnerable;
