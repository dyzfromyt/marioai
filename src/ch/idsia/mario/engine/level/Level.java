...
public class Level implements Copyable
{
    ...
    {
        Level result = new Level(parent);
        result.width = width;
        result.height = height;
        result.xExit = xExit;
        result.yExit = yExit;

        result.map = new byte[width][height];
        result.data = new byte[width][height];
        result.spriteTemplates = new SpriteTemplate[width][height];
        result.observation = new byte[width][height];
        ...
        for(int i = 0 ; i < width ; i ++)
        {
            for (int j = 0 ; j < height; j ++)
            {
                ...
                result.observation[i][j] = observation[i][j];
                if(spriteTemplates[i][j] == null)
                {
                    result.spriteTemplates[i][j] = null;
                }
                else
                {
                    result.spriteTemplates[i][j] = (SpriteTemplate) (spriteTemplates[i][j].makeTrainingCopy(parent));
                }
            }
        }
        return result;
    }
    protected Level(Object newParent)
    {
    }
...
//    }
    ...
    public void saveTxt(Writer doc) throws IOException
    {
        StringBuilder sb = new StringBuilder();
        sb.append(Level.FILE_HEADER);
        sb.append("\n");
        sb.append((byte) 0);
        sb.append("\n");
        sb.append((short) width);
        sb.append("\n");
        sb.append((short) height);
        sb.append("\n");
        for (int y = 0; y < height; y++)
        {
            for (int i = 0; i < width; i++)
    ...
    public void setSpriteTemplate(int x, int y, SpriteTemplate spriteTemplate)
    {
        ...
        spriteTemplate.thisX = x;
        spriteTemplate.thisY = y;
    }
