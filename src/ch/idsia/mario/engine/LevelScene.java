...
public class LevelScene extends Scene implements SpriteContext, Copyable
{
    ...
    public LevelScene(GraphicsConfiguration graphicsConfiguration, MarioComponent renderer, long seed, int levelDifficulty, int type, int levelLength, int timeLimit)
    {
        ...
        this.killedCreaturesTotal = 0;
        this.killedCreaturesByFireBall = 0;
        this.killedCreaturesByStomp = 0;
        this.killedCreaturesByShell = 0;
    }
...
//                        return -12; // hard formation border. Pay attention!
    ...
    public byte[][] levelSceneObservation(int ZLevel)
    {
        ...
        for (int x = MarioXInMap - Environment.HalfObsWidth, obsX = 0; x < MarioXInMap + Environment.HalfObsWidth; x++, obsX++)
        {
            for (int y = MarioYInMap - Environment.HalfObsHeight, obsY = 0; y < MarioYInMap + Environment.HalfObsHeight; y++, obsY++)
            {
                // ERROR: obsX is Y, obsY is X
...
//        ret[Environment.HalfObsWidth][Environment.HalfObsHeight] = mario.kind;
        for (Sprite sprite : sprites)
        {
            ...
            if (sprite.mapX >= 0 &&
                ...
                sprite.mapY < MarioYInMap + Environment.HalfObsHeight )
            {
                int obsY = sprite.mapY - MarioYInMap + Environment.HalfObsHeight;
                int obsX = sprite.mapX - MarioXInMap + Environment.HalfObsWidth;
    ...
    public byte[][] mergedObservation(int ZLevelScene, int ZLevelEnemies)
    {
        ...
        for (int x = MarioXInMap - Environment.HalfObsWidth, obsX = 0; x < MarioXInMap + Environment.HalfObsWidth; x++, obsX++)
        {
            for (int y = MarioYInMap - Environment.HalfObsHeight, obsY = 0; y < MarioYInMap + Environment.HalfObsHeight; y++, obsY++)
            {
                if (x >=0 /*&& x <= level.xExit*/ && y >= 0 && y < level.height)
                {
                    byte elementCode = ZLevelMapElementGeneralization(level.map[x][y], ZLevelScene);
                    ret[obsX][obsY] = elementCode;
                }
                else
                    ret[obsX][obsY] = 0;
//                if (x == MarioXInMap && y == MarioYInMap)
...
//                }
    ...
    public static List<Sprite> makeTrainCopySprites(LevelScene world, LevelScene oldWorld, List<Sprite> source)
    {
        List<Sprite> result = new ArrayList<Sprite>();
        // TODO, this is a special case for mario carried, think of something better
        if(oldWorld.mario.carried!=null)
        {
            if(source.contains(oldWorld.mario.carried))
            {
                source.remove(oldWorld.mario.carried);
                Sprite newCarried = (Sprite) oldWorld.mario.carried.makeTrainingCopy(world);
                result.add(newCarried);
                world.mario.carried = newCarried;
            }
        }
        
        for(Sprite src : source)
        {
            if(src instanceof Mario)
            {
                // mario already copied, just add it in
                result.add(world.mario);
            }
            else
            {
                result.add((Sprite)src.makeTrainingCopy(world));
            }
        }
        return result;
    }
    ...
    {
        return makeTrainingCopy(parent, this);
    }
    
    private static LevelScene makeTrainingCopy(Object newParent, LevelScene source)
    {
        LevelScene result=new LevelScene();
        result.level =(Level)(source.level.makeTrainingCopy(result));

        result.graphicsConfiguration = source.graphicsConfiguration;
        result.levelSeed = source.levelSeed;
        result.renderer = source.renderer;
        result.levelDifficulty = source.levelDifficulty;
        result.levelType = source.levelType;
        result.levelLength = source.levelLength;
        result.setTotalTime(source.totalTime);
        result.killedCreaturesTotal = source.killedCreaturesTotal;
        result.killedCreaturesByFireBall = source.killedCreaturesByFireBall;
        result.killedCreaturesByStomp = source.killedCreaturesByStomp;
        result.killedCreaturesByShell = source.killedCreaturesByShell;

        result.paused = source.paused;
        //Sprite.spriteContext = result;
        //sprites.clear();
        //layer = new LevelRenderer(level, graphicsConfiguration, 320, 240);
        result.layer = source.layer; // TO DO: make sure this is ok, need to separate game dynamics and graphics
        for (int i = 0; i < 2; i++)
        {
//            int scrollSpeed = 4 >> i;
...
//            bgLayer[i] = new BgRenderer(bgLevel, graphicsConfiguration, 320, 240, scrollSpeed);
            result.bgLayer[i] = null; // explicitly set to null to make sure no one does rendering on training copies
        }
        
        result.mario = (Mario)source.mario.makeTrainingCopy(result);
        result.sprites=makeTrainCopySprites(result, source, source.sprites);

        result.spritesToAdd =makeTrainCopySprites(result, source, source.spritesToAdd);
        result.spritesToRemove =makeTrainCopySprites(result, source, source.spritesToRemove);
                ...
                for (int y = (int) yCam / 16 - 1; y <= (int) (yCam + layer.height) / 16 + 1; y++)
                {
                    ...
                    if (st != null)
                    {
                        if (st.lastVisibleTick != tick - 1)
                        {
                            ...
                            //if (Template not instantiated yet || instantiated sprite died )
                            if(!st.isInstantiated)
                            {
...
//            System.err.println("warning: progress line inaccuracy");
    ...
    public void setTemplateDead(int templateX  , int templateY)
    {
        if(level.spriteTemplates[templateX][templateY] != null)
        {
            level.spriteTemplates[templateX][templateY].isDead = true;
        }
    }

    public void removeSprite(Sprite sprite)
    {
        if(level.spriteTemplates[sprite.templateX][sprite.templateY] != null)
        {
            level.spriteTemplates[sprite.templateX][sprite.templateY].isInstantiated = false;
        }
