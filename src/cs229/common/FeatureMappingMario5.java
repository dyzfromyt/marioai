 ...
 * @author Stephen
 */
public class FeatureMappingMario5 implements FeatureMapping
{
    ...
    public byte[] map(Environment observation)
    {
        ...
        return map(state, halfObsWidth, halfObsHeight, marioMode, mayMarioJump, isMarioOnGround, observation );
    }
    ...
    public byte[] map(byte[][] state, int halfObsWidth, int halfObsHeight, int marioMode, boolean mayMarioJump, boolean isMarioOnGround, Environment observation)
    {
        byte[] result = new byte[featureElementRange.length];
...
//        result[counter++]=(byte)(pc4); //5
        ...
        result[counter++] = (byte)observation.getMarioMode();
