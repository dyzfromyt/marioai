 ...
 * @author Stephen
 */
public class FeatureMappingMario3 implements FeatureMapping {
    ...
    public static int [] featureElementRange={2,2, 2,2,2,2,2,2,2, 4, 4,4,4,4};
    ...
    private final int NumHoleCheck = 7;
...
//        result[counter++]=(byte)(pc4); //5
        ...
        result[counter++]=(byte)enemyDistance(state, marioPosX+1, marioPosY); //6
    ...
    public static final int FEATURE_ENEMY_DIST = 9;
    public double reward(byte[] feature, MarioComponent trueState, int action)
    {
        ...
        if(feature[0] == 0 && feature[1] == 1 && action ==ActionMappingMario2.JUMP)
        {
            reward = -3;
        }
        // cannot jump, in air, release press jump = -3
        if (feature[0] == 0 && feature[1] == 0 && action != ActionMappingMario2.JUMP)
        {
            reward = -3;
        }

        
        reward += feature[FEATURE_ENEMY_DIST];
